...
```
java -Dspring.profiles.active=local -jar ./target/icms-0.0.1-SNAPSHOT.jar
java -Dspring.profiles.active=production -jar ./target/icms-0.0.1-SNAPSHOT.jar
```
...
```
mvnw spring-boot:build-image -Dspring-boot.build-image.imageName=com.bcom/icms
```

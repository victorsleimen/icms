...
export default function PriorityMatrixAdd() {
  const { t } = useTranslation();
  useDocumentTitle(t('priorityMatrix.add.headline'));
  ...
  const createPriorityMatrix = async (data: PriorityMatrixDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.post('/api/priorityMatrices', data);
      navigate('/priorityMatrices', {
            state: {
              msgSuccess: t('priorityMatrix.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('priorityMatrix.add.headline')}</h1>
      <div>
        <Link to="/priorityMatrices" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('priorityMatrix.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createPriorityMatrix)} noValidate>
      ...
      <input type="submit" value={t('priorityMatrix.add.headline')} className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2 mt-6" />
    </form>
  </>);
}
